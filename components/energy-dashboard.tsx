...
function PageHeading({ eyebrow, title, subtitle, badge }: { eyebrow: string; title: string; subtitle: string; badge?: string }) {
  return <section className="page-heading"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p>{subtitle}</p></div><div className="heading-actions">{badge && <span className="data-badge"><i />{badge}</span>}</div></section>;
}
...
function PortfolioView({ navigate }: { navigate: Navigate }) {
  return <>
    ...
    <section className="metric-grid"><Kpi icon={SunMedium} label="Solar generated" value={fmt(totals.solarEnergyMwh,3)} unit="MWh" note="Solar Total · electricity_energy_power" tone="good" /><Kpi icon={Zap} label="Estimated site load" value={fmt(totals.estimatedLoadMwh,3)} unit="MWh" note="Grid import + solar generation" /><Kpi icon={Sparkles} label="Generation at PPA rate" value={`R ${fmt(totals.avoidedCostZar,0)}`} note={`Rate exposure only · R ${site.tariff.toFixed(2)}/kWh`} tone="good" /><Kpi icon={Activity} label="Data availability" value="100.0" unit="%" note="Latest complete aligned day" tone="good" /></section>

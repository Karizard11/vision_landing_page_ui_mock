...
import { periodResolutionLabel } from "@/lib/period-resolution";
...
function DateSelector({ range, onChange }: { range: DateRange; onChange: (range: DateRange) => void }) {
  ...
  return <div className="date-navigation"><button onClick={() => shift(-1)} aria-label="Previous period"><ChevronLeft/></button><Popover open={open} onOpenChange={value => { setOpen(value); if (value) setDraft(range); }}>
    ...
    </div><div className="date-picker-actions"><button className="apply" onClick={() => { if (draft?.from) onChange({from:draft.from,to:draft.to ?? draft.from}); setOpen(false); }}>Apply</button><button onClick={() => setDraft({from:anchorDate,to:anchorDate})}>Reset</button><span>1 day: 5 min · 2–4 days: 30 min · 5–14 days: hourly · 15–31 days: daily</span></div></PopoverContent>
...
function PortfolioView({ navigate, period }: { navigate: Navigate; period: PrecoolPeriod }) {
  ...
  const chartUnit = daily ? "MWh/day" : "kW";
  ...
  return <><PageTitle title="Terradew Four" subtitle={`${portfolioTotals.sites} sites | ${portfolioTotals.meters} meters | Doris as of ${dataAsOf}`} action={false}/>
    ...
    <div className="portfolio-overview"><article className="panel exposure"><div className="panel-head"><strong>Exposure</strong><span>portfolio</span></div><div><Check/><span>Sites with active VCOM systems</span><b>23 of 23</b></div><div><AlertTriangle/><span>Sites below 95% guarantee</span><b>1 of 23</b></div><div><Database/><span>Physical meters in SLD</span><b>129</b></div></article>
      <ChartPanel title="Yield, expectation and grid use | P0480 PreCool" hint={`${selectedPeriodLabel(period)} · ${resolution} · ${chartUnit}`}>
        {daily ? <ResponsiveContainer width="100%" height="100%"><BarChart data={period.power} margin={{...chartMargin,bottom:8}}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:8}}/><Bar dataKey="grid" name="Grid supply MWh" fill="#31808a"/><Bar dataKey="solar" name="Metered solar MWh" fill="#ff806c"/><Bar dataKey="expected" name="Solcast expectation MWh" fill="#83b48b"/></BarChart></ResponsiveContainer>
          : <ResponsiveContainer width="100%" height="100%"><AreaChart data={period.power} margin={{...chartMargin,bottom:8}}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:8}}/><Area type="monotone" stackId="supply" dataKey="grid" name={`Grid supply ${chartUnit}`} stroke="#286f79" fill="#31808a" fillOpacity={0.62} dot={false}/><Area type="monotone" stackId="supply" dataKey="solar" name={`Metered solar ${chartUnit}`} stroke="#f15b45" fill="#ff806c" fillOpacity={0.72} dot={false}/><Area type="monotone" dataKey="expected" name={`Solcast expectation ${chartUnit}`} stroke="#579363" fill="#66a26f" fillOpacity={0.1} strokeDasharray="5 4" dot={false}/></AreaChart></ResponsiveContainer>}
      </ChartPanel>
    </div>
...
function SiteView({ item, navigate, period }: { item: PortfolioSite; navigate: Navigate; period: PrecoolPeriod }) {
  ...
  return <><PageTitle title={item.name} subtitle={`${item.code} | ${item.city} | ${num(item.capacityKwp/1000,2)}MWp | commissioned ${item.commissioned}`}/>
    ...
    <div className="site-chart-row"><ChartPanel title={isPrecool ? `Site overview | ${chartUnit}` : "Contract yield profile (MWh)"} hint={isPrecool ? `${selectedPeriodLabel(period)} · ${resolution}` : undefined}>{isPrecool ? daily ? <ResponsiveContainer width="100%" height="100%"><BarChart data={period.power} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Bar dataKey="grid" name="Grid supply MWh" fill="#31808a"/><Bar dataKey="solar" name="Solar production MWh" fill="#65ba75"/></BarChart></ResponsiveContainer> : <ResponsiveContainer width="100%" height="100%"><AreaChart data={period.power} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Area dataKey="grid" name={`Grid supply ${chartUnit}`} stroke="#185c68" fill="#185c6870"/><Area dataKey="solar" name={`Solar ${chartUnit}`} stroke="#55a96f" fill="#65ba7580"/></AreaChart></ResponsiveContainer> : <ResponsiveContainer width="100%" height="100%"><BarChart data={monthPlan(item)} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="month" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Bar dataKey="plan" name="Monthly yield plan MWh" fill="#dfe6e7"/><Bar dataKey="actual" name="Contract-derived profile MWh" fill="#249b61"/></BarChart></ResponsiveContainer>}</ChartPanel>
...
function MeterView({ item, node, navigate, period }: { item: PortfolioSite; node: PortfolioNode; navigate: Navigate; period: PrecoolPeriod }) {
  ...
  return <><PageTitle title={node.name} subtitle={`${item.name} | ${node.type} | Device node ${node.id}`}/>
    ...
    <ChartPanel title={solar ? "Solar and grid profile" : "Energy profile"} hint={hasLiveMeterData ? `${selectedPeriodLabel(period)} · ${resolution} · ${chartUnit}` : "contract-derived monthly plan"}>{hasLiveMeterData && daily ? <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/>{key !== "grid" && <Bar dataKey="grid" name="Grid supply MWh" fill="#dfe5e6"/>}<Bar dataKey={key} name={`${node.name} MWh`} fill={solar ? "#249b61" : "#185c68"}/></BarChart></ResponsiveContainer> : hasLiveMeterData && solar ? <ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Line dataKey={key} name={`${node.name} ${chartUnit}`} stroke="#f0ad42" strokeWidth={2} dot={false}/><Line dataKey="expected" name={`Solcast expectation ${chartUnit}`} stroke="#ef705f" strokeDasharray="4 3" dot={false}/><Line dataKey="grid" name={`Grid total ${chartUnit}`} stroke="#526f76" strokeWidth={1.4} dot={false}/></ComposedChart></ResponsiveContainer> : hasLiveMeterData ? <ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Line dataKey={key} name={`${node.name} ${chartUnit}`} stroke="#185c68" strokeWidth={2} dot={false}/>{key !== "grid" && <Line dataKey="grid" name={`Grid total ${chartUnit}`} stroke="#90a3a6" strokeWidth={1.2} dot={false}/>}</LineChart></ResponsiveContainer> : <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="month" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Bar dataKey="plan" fill="#dfe5e6"/><Bar dataKey="actual" fill={solar ? "#249b61" : "#185c68"}/></BarChart></ResponsiveContainer>}</ChartPanel>
...
function InverterTotalView({ navigate, period }: { navigate: Navigate; period: PrecoolPeriod }) {
  const daily = period.granularity === "day";
  const chartUnit = daily ? "daily peak power (kW)" : periodResolutionLabel(period.granularity) + " power (kW)";
  ...
  return <><PageTitle title="Inverter total" subtitle={`PreCool Cold Storage | Solar Total | ${period.inverterSummary.length} Sungrow SG125CX-P2 inverters`}/><CoverageNotice period={period}/><div className="kpi-grid meter-four-kpis"><Kpi icon={Gauge} label="AC peak" value={period.totals.inverterReadings ? num(period.totals.peakAcMw,3) : "—"} unit={period.totals.inverterReadings ? "MW" : undefined} note="Summed inverter output" delta={period.totals.inverterReadings ? "measured" : undefined} tone={period.totals.inverterReadings ? "green" : undefined}/><Kpi icon={Zap} label="Energy" value={period.totals.inverterReadings ? num(period.totals.inverterEnergyMwh,3) : "—"} unit={period.totals.inverterReadings ? "MWh" : undefined} note={selectedPeriodLabel(period)}/><Kpi icon={Database} label="Cumulative energy" value={period.totals.cumulativeEnergyGwh !== null ? num(period.totals.cumulativeEnergyGwh,3) : "—"} unit={period.totals.cumulativeEnergyGwh !== null ? "GWh" : undefined} note="Latest E_TOTAL in selection"/><Kpi icon={Activity} label="Availability" value={num(period.totals.inverterAvailability,1)} unit="%" note={`${reporting} / 12 units · ${period.totals.inverterReadings.toLocaleString("en-ZA")} readings`} delta={period.inverterCoverage === "complete" ? "online" : "partial"} tone={period.inverterCoverage === "complete" ? "green" : "amber"}/></div>
    <ChartPanel title="All inverter power" hint={`AC · ${chartUnit}`}><ResponsiveContainer width="100%" height="100%">{daily ? <BarChart data={period.inverterAc} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:10}}/>{period.inverterSummary.map((inv,index) => <Bar key={inv.code} dataKey={`i${inv.code}`} name={`Inv ${inv.code}`} fill={inverterColours[index]}/>)}</BarChart> : <LineChart data={period.inverterAc} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:10}}/>{period.inverterSummary.map((inv,index) => <Line key={inv.code} dataKey={`i${inv.code}`} name={`Inv ${inv.code}`} stroke={inverterColours[index]} strokeWidth={1.8} dot={false} connectNulls={false}/>)}</LineChart>}</ResponsiveContainer></ChartPanel>
    <ChartPanel title="All inverter DC power" hint={`P_DC · ${chartUnit}`}><ResponsiveContainer width="100%" height="100%">{daily ? <BarChart data={period.inverterDc} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:10}}/>{period.inverterSummary.map((inv,index) => <Bar key={inv.code} dataKey={`i${inv.code}`} name={`Inv ${inv.code}`} fill={inverterColours[index]}/>)}</BarChart> : <LineChart data={period.inverterDc} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:10}}/>{period.inverterSummary.map((inv,index) => <Line key={inv.code} dataKey={`i${inv.code}`} name={`Inv ${inv.code}`} stroke={inverterColours[index]} strokeWidth={1.8} dot={false} connectNulls={false}/>)}</LineChart>}</ResponsiveContainer></ChartPanel>
...
function SingleInverterView({ code, period }: { code: string; period: PrecoolPeriod }) {
  ...
  const irradianceUnit = period.granularity === "day" ? "kWh/m²/day" : "W/m²";
  const chartUnit = period.granularity === "day" ? "daily peak power" : periodResolutionLabel(period.granularity) + " power";

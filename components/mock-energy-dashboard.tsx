...
function p0480MeterId(nodeId: string) {
  return ({ "1140723":"solar-total", "1140730":"pvdb-1", "1140721":"pvdb-2", "1140727":"municipal-total", "1140724":"incomer-1", "1140728":"incomer-2", "1140729":"incomer-3" } as Record<string,string>)[nodeId];
}

function isVirtualTotalNode(node: PortfolioNode) {
  const type = node.type.toLowerCase();
  return type === "solar total" || type === "municipal total";
}
...
function MeterView({ item, node, navigate, period }: { item: PortfolioSite; node: PortfolioNode; navigate: Navigate; period: PrecoolPeriod }) {
  ...
  const key = meterId === "pvdb-1" ? "pvdb1" : meterId === "pvdb-2" ? "pvdb2" : meterId === "solar-total" ? "solar" : meterId === "municipal-total" ? "grid" : meterId?.replace("-","");
  ...
  const chartData = hasLiveMeterData ? period.power : monthPlan(item);
  ...
  const chartUnit = period.granularity === "hour" ? "kW" : "MWh/day";
  const meterAvailability = meterSnapshot ? meterSnapshot.readings/(period.dayCount*288*(key === "solar" ? 2 : key === "grid" ? 3 : 1))*100 : 0;

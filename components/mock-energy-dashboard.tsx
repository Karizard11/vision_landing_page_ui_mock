...
function PortfolioView({ navigate, period }: { navigate: Navigate; period: PrecoolPeriod }) {
  ...
  const energyBalance = period.power.map(point => ({
    ...point,
    siteLoad: point.grid + point.solar,
  }));
  ...
  return <><PageTitle title="Terradew Four" subtitle={`${portfolioTotals.sites} sites | ${portfolioTotals.meters} meters | Doris as of ${dataAsOf}`} action={false}/>
    ...
    <div className="portfolio-overview"><article className="panel exposure"><div className="panel-head"><strong>Exposure</strong><span>portfolio</span></div><div><Check/><span>Sites with active VCOM systems</span><b>23 of 23</b></div><div><AlertTriangle/><span>Sites below 95% guarantee</span><b>1 of 23</b></div><div><Database/><span>Physical meters in SLD</span><b>129</b></div></article>
      <ChartPanel title="Yield, expectation and grid use | P0480 PreCool" hint={`${selectedPeriodLabel(period)} · ${chartUnit}`}><ResponsiveContainer width="100%" height="100%"><LineChart data={energyBalance} margin={{...chartMargin,bottom:8}}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:8}}/><Line dataKey="siteLoad" name={`Estimated site demand ${chartUnit}`} stroke="#183f48" strokeWidth={2.2} dot={false}/><Line dataKey="grid" name={`Grid supply ${chartUnit}`} stroke="#31808a" strokeWidth={2} dot={false}/><Line dataKey="solar" name={`Metered solar ${chartUnit}`} stroke="#ff5a43" strokeWidth={2} dot={false}/><Line dataKey="expected" name={`Solcast expectation ${chartUnit}`} stroke="#66a26f" strokeDasharray="5 4" strokeWidth={1.5} dot={false}/></LineChart></ResponsiveContainer></ChartPanel>
    </div>

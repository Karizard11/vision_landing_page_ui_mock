    ...
    <section className="white-navigation">
      ...
      <div className="navigation-tree">
        ...
        {visibleSites.map(item => { const activeSite = view.kind !== "portfolio" && view.siteCode === item.code; const isOpen = expanded.has(item.code) || activeSite; return <div className="site-tree" key={item.code}>
          ...
          {isOpen && <div className="site-node-list">{item.nodes.map(node => { const activeNode = view.kind === "meter" && view.siteCode === item.code && view.nodeId === node.id; return <button key={`${item.code}-${node.id}`} style={{paddingLeft:20 + nodeDepth(node,item.nodes) * 14}} className={`node-row ${activeNode ? "active" : ""}`} onClick={() => navigate({kind:"meter",siteCode:item.code,nodeId:node.id})}><ChevronRight/><NodeIcon type={node.type}/><span>{node.name}</span>{node.meters > 1 && <small>{node.meters}</small>}</button>; })}
          {item.code === "P0480" && <><button className={`node-row inverter-node ${view.kind === "inverters" ? "active" : ""}`} onClick={() => navigate({kind:"inverters",siteCode:"P0480"})}><ChevronRight/><Layers3/><span>Inverter total</span><small>12</small></button><button className={`node-row inverter-unit ${view.kind === "inverter" ? "active" : ""}`} onClick={() => navigate({kind:"inverter",siteCode:"P0480",inverterCode:"01"})}><ChevronRight/><Gauge/><span>Inverter 001</span></button></>}
          </div>}
        </div>; })}
      </div>
    </section>
...
function PortfolioView({ navigate }: { navigate: Navigate }) {
  return <><PageTitle title="Terradew Four" subtitle={`${portfolioTotals.sites} sites | ${portfolioTotals.meters} meters | Last updated ${site.lastDorisRefresh}`} action={false}/>
    <div className="kpi-grid portfolio-kpis"><Kpi icon={SunMedium} label="Precool solar produced" value={num(totals.solarEnergyMwh,3)} unit="MWh" note={`${site.snapshotDate} complete day`} delta="metered" tone="green"/><Kpi icon={Database} label="Contracted annual yield" value={num(portfolioTotals.annualYieldGwh,2)} unit="GWh" note="mv_contracts_solar aggregation"/><Kpi icon={Gauge} label="Installed capacity" value={num(portfolioTotals.capacityMwp,2)} unit="MWp" note={`${portfolioTotals.sites} Terradew Four contracts`} delta="active" tone="green"/><Kpi icon={Activity} label="Mapped meter assets" value={String(portfolioTotals.meters)} note={`Weighted guarantee ${num(portfolioTotals.weightedGuarantee,1)}%`} delta="SLD" tone="green"/></div>
...
function SiteView({ item, navigate }: { item: PortfolioSite; navigate: Navigate }) {
  ...
  return <><PageTitle title={item.name} subtitle={`${item.code} | ${item.city} | ${num(item.capacityKwp/1000,2)}MWp | commissioned ${item.commissioned}`}/>
    ...
    <div className="kpi-grid savings-kpis"><Kpi icon={SunMedium} label="Solar produced" value={isPrecool ? num(totals.solarEnergyMwh,3) : num(item.annualYieldKwh/1000,0)} unit={isPrecool ? "MWh" : "MWh/y"} note={isPrecool ? "electricity_energy_power" : "contracted annual yield"} delta="solar" tone="green"/><Kpi icon={Database} label="PPA value" value={isPrecool ? `R ${num(totals.avoidedCostZar,0)}` : item.tariff ? `R ${num(item.tariff,2)}` : "—"} note={isPrecool ? `at R ${num(item.tariff ?? 0,2)}/kWh` : "current PPA rate"}/><Kpi icon={Network} label="SLD coverage" value={`${item.nodeCount} nodes`} note={`${item.meterCount} physical meters`} delta="Doris" tone="green"/><Kpi icon={Activity} label="Yield baseline" value={num(item.annualYieldKwh/1_000_000,2)} unit="GWh/y" note="mv_contracts_solar"/></div>
...
function MeterView({ item, node, navigate }: { item: PortfolioSite; node: PortfolioNode; navigate: Navigate }) {
  ...
  return <><PageTitle title={node.name} subtitle={`${item.name} | ${node.type} | Device node ${node.id}`}/>
    <div className={`kpi-grid ${solar ? "meter-five-kpis" : "meter-four-kpis"}`}>{solar ? <><Kpi icon={Zap} label="Generating now" value={meter ? num(meter.peakKw,1) : num(item.capacityKwp,1)} unit="kW" note={meter ? "peak on selected day" : "contract capacity"} delta="active" tone="green"/><Kpi icon={Database} label="Produced today" value={meter ? num(meter.energyMwh*1000,1) : num(item.annualYieldKwh/365,1)} unit="kWh" note={meter ? "export register delta" : "daily yield plan"} delta="energy"/><Kpi icon={Gauge} label="Performance ratio" value={item.code === "P0480" ? num(totals.prEstimate,1) : num(item.guarantee,1)} unit="%" note="against contract baseline" delta="8.8%" tone="green"/><Kpi icon={Activity} label="Avoided cost" value={meter && item.tariff ? `R ${num(meter.energyMwh*1000*item.tariff,0)}` : item.tariff ? `R ${num(item.tariff,2)}` : "—"} note={meter ? "energy × PPA rate" : "current PPA rate"} delta="value" tone="green"/><Kpi icon={Database} label="Physical meters" value={String(node.meters)} note={`${connected.length} child nodes`}/></> : <><Kpi icon={Zap} label="Imported today" value={meter ? num(meter.energyMwh*1000,1) : "—"} unit={meter ? "kWh" : undefined} note={meter ? "import register delta" : "interval data not wired"}/><Kpi icon={Gauge} label="Maximum demand" value={meter ? num(meter.peakKw,1) : num(item.capacityKwp,1)} unit="kW" note={meter ? "peak absolute ptot" : "site DC capacity"}/><Kpi icon={Activity} label="Data completeness" value={meter ? "100.0" : "SLD"} unit={meter ? "%" : undefined} note={meter ? "288 five-minute readings" : "metadata available"} tone="green"/><Kpi icon={Database} label="Mapped meters" value={String(node.meters)} note={`Node ${node.id}`}/></>}</div>
    <ChartPanel title={solar ? "Import and export" : "Energy produced"} hint={meter ? `${site.snapshotDate} · 5-minute source aggregated hourly` : "contract-derived monthly plan"}>{meter ? <ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Line dataKey={key} name={`${node.name} kW`} stroke={solar ? "#f0ad42" : "#185c68"} strokeWidth={2} dot={false}/>{solar && <Line dataKey="expected" name="Solcast expectation" stroke="#ef705f" strokeDasharray="4 3" dot={false}/>}<Line dataKey="grid" name="Grid total kW" stroke="#526f76" strokeWidth={1.4} dot={false}/></ComposedChart></ResponsiveContainer> : <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="month" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Bar dataKey="plan" fill="#dfe5e6"/><Bar dataKey="actual" fill={solar ? "#249b61" : "#185c68"}/></BarChart></ResponsiveContainer>}</ChartPanel>
    <section className="all-meters"><div className="section-label"><strong>{item.code === "P0480" && solar ? "Inverters" : connected.length ? "Child nodes" : "Related nodes"}</strong><span>{item.name}</span></div>{item.code === "P0480" && solar ? <div className="meter-card-grid"><button className="meter-card" onClick={() => navigate({kind:"inverters",siteCode:"P0480"})}><div><Layers3/><strong>Inverter total</strong></div><span>12 Sungrow units</span><small>{num(totals.inverterEnergyMwh,3)} MWh</small><em>Open</em></button>{inverterSummary.slice(0,3).map(inv => <button className="meter-card" key={inv.code} onClick={() => navigate({kind:"inverter",siteCode:"P0480",inverterCode:inv.code})}><div><Gauge/><strong>Inverter {inv.code.padStart(3,"0")}</strong></div><span>{inv.model}</span><small>{num(inv.energy,1)} kWh</small><em>Active</em></button>)}</div> : <div className="meter-card-grid">{(connected.length ? connected : item.nodes.filter(value => value.id !== node.id).slice(0,6)).map(value => <NodeCard key={value.id} node={value} item={item} navigate={navigate}/>)}</div>}</section>
  </>;
}
...
function InverterTotalView({ navigate }: { navigate: Navigate }) {
  return <><PageTitle title="Inverter total" subtitle={`PreCool Cold Storage | Solar Total | ${inverterSummary.length} Sungrow SG125CX-P2 inverters`}/><div className="kpi-grid meter-four-kpis"><Kpi icon={Gauge} label="AC power" value={num(totals.peakAcMw,3)} unit="MW" note="summed peak output" delta="all units" tone="green"/><Kpi icon={Zap} label="Energy today" value={num(totals.inverterEnergyMwh,3)} unit="MWh" note="sum of E_DAY"/><Kpi icon={Database} label="Cumulative energy" value={num(totals.cumulativeEnergyGwh,3)} unit="GWh" note="sum of E_TOTAL"/><Kpi icon={Activity} label="Availability" value="12 / 12" note="3,456 readings" delta="online" tone="green"/></div>
    <ChartPanel title="All inverter power stacked" hint="AC power (kW)"><ResponsiveContainer width="100%" height="100%"><AreaChart data={inverterStack} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:10}}/>{inverterSummary.map((inv,index) => <Area key={inv.code} dataKey={`i${inv.code}`} name={`Inv ${inv.code}`} stackId="1" stroke={inverterColours[index]} fill={inverterColours[index]} fillOpacity={.82}/>)}</AreaChart></ResponsiveContainer></ChartPanel>
...
function SingleInverterView({ code }: { code: string }) {
  ...
  return <><PageTitle title={`Inverter ${inv.code.padStart(3,"0")}`} subtitle={`PreCool Cold Storage | ${inv.id} | ${inv.model}`}/><div className="kpi-grid meter-four-kpis"><Kpi icon={Gauge} label="AC power" value={num(inv.peakAc,1)} unit="kW" note="day peak" delta="active" tone="green" bars/><Kpi icon={Zap} label="Energy today" value={num(inv.energy,1)} unit="kWh" note="E_DAY register" bars/><Kpi icon={Activity} label="Conversion" value={num(ratio,1)} unit="%" note="peak AC ÷ DC" tone="green" bars/><Kpi icon={Database} label="Cumulative energy" value={num(inv.cumulative/1000,1)} unit="MWh" note="E_TOTAL register" bars/></div>
    <div className="inverter-detail-row"><ChartPanel title="Actual against modelled output" hint="AC, DC and string sum"><ResponsiveContainer width="100%" height="100%"><AreaChart data={singleInverterSeries} margin={chartMargin}><CartesianGrid stroke="#dbe5e6" vertical={false}/><XAxis dataKey="time" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Area dataKey="dc" name="DC power" stroke="#f0ab40" fill="#f0ab4025"/><Line dataKey="ac" name="AC power" stroke="#ef654b" strokeWidth={2} dot={false}/><Line dataKey="strings" name="String sum" stroke="#235f69" strokeDasharray="4 3" dot={false}/></AreaChart></ResponsiveContainer></ChartPanel><article className="panel string-status"><div className="panel-head"><strong>String telemetry</strong><span>at peak</span></div><div className="string-tile-grid">{stringData.map((string,index) => <div key={string.name}><span>{string.name}</span><strong>{num(string.power,1)}<small> kW</small></strong><em>{index < 2 ? "high" : "normal"}</em></div>)}</div><footer><span>9 / 9 strings reporting</span><b>{num(stringData.reduce((sum,value) => sum + value.power,0),1)} kW sum</b></footer></article></div>

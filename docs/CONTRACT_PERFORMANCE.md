...
Timestamp interpretation is source-specific. `pv_model.timestamp`, `pv_sol.timestamp`, and `mv_pv_model_forecasts.contract_timestamp` use **the contract's local wall time**. Resolve its IANA `contractTimeZone`, query dated forecasts using contract-local bounds, and match profiles by the contract-local month/day/hour without interpreting naive model timestamps as UTC. Terradew Four and Redefine Properties are confirmed as `Africa/Johannesburg` (SAST). Meter and Solcast timestamps remain **UTC** and are converted once for SAST display/bucketing. For a SAST contract, a 10:00 SAST chart interval uses the 10:00 PVModel/PVSOL values and the 08:00 UTC meter interval; Solcast uses the corresponding period ending at 08:30 UTC. The selected ending minute is inclusive; a selection ending at 10:30 uses an exclusive boundary of 10:31. Forecasts are hourly even when the display has five-minute points: shorter points are allocations of the hourly model, not additional source observations.

The catalog carries `contractTimeZone`; an explicit contract setting overrides the confirmed portfolio setting. The current Doris contract view has no IANA timezone column, so only the two confirmed portfolios default to SAST. Other contracts require an explicit valid IANA timezone; do not infer it from a VCOM numeric UTC offset or silently assume SAST. Named zones account for seasonal offsets. Hourly UTC actuals are proportionally allocated when they straddle contract-local model-hour boundaries. API display/range times remain SAST.
...
The primary page presents four contract metrics plus a compact site-operations summary, one energy chart, and irradiation/PR charts. Gains/losses, contract data notes, and individual meters/loads are expandable to keep the working view compact. There is no waterfall chart.

## Validation

Regression coverage includes contract-local model/UTC telemetry alignment, SAST defaults for the two confirmed portfolios, explicit timezone overrides, seasonal and half-hour offsets, missing timezone validation, local-midnight/year boundaries, forecast query bounds, partial-minute windows, single degradation application, missing versus zero energy, matched comparisons, overlapping forecasts, missing irradiance, leap-day gaps, monthly sums and recomputed PR, and unique contract navigation. Live checks cover PreCool and Redefine, including meter timestamp jitter and longer date ranges. Doris is read-only throughout.
